...
float observation_model(vector<float> landmark_positions, 
  ...
  // YOUR CODE HERE
  // i curent psuedo_position in main_func
  for(int z=0;z<observations.size();z++)
  {
      float pseudo_range_min;
      //determine if a pseudo range vector exists for the current pseudo position x
      //if the vector exists, extract and store the minimum distance, 
      //element 0 of the sorted vector,and remove that element
      // (so we don't re-use it). This will be passed to norm_pdf
      if (pseudo_ranges.size()>0)
      {
          //set min distance
          pseudo_range_min=pseudo_ranges[0];
          //remove this entry form pseudo_range-vectors
